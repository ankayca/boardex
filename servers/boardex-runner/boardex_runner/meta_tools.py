"""Harness meta-tools (RUNNER_AGENT_V0_SPEC §4): structured product events are
emitted by tool call, never parsed out of prose. Each payload is validated
against these schemas (and the contract shapes) by the agent engine; a
malformed payload fails visibly, the model is told why, and gets exactly one
retry per tool before the run aborts as failed (§3.5 fail-closed).
"""
