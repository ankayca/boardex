...
def inspect_peripheral(
    ...
    Args:
        ...
        peripheral: Registered name, e.g. ``"I2C1"`` or ``"I2C2"``. Call with an
            unknown name to get ``data.supported`` listing what's available.
    ...
    return _guard(
        lambda: inspect_fn(device_id, peripheral, target=target)
    ).to_dict()
...
def run_checkpoint(
    ...
    inspect_on_failure: str | None = "I2C1",
    elf_path: str | None = None,
) -> dict[str, Any]:
    ...
    On RTT timeout, optionally inspects a peripheral (default ``I2C1``) and
    attaches decoded registers/pins/hints to ``data.evidence``.

    Branch on ``data.evidence.verdict`` and ``data.evidence.rtt.matched``.
    """
    return _guard(
        lambda: workflows.run_checkpoint(
            registry,
            sessions,
            device_id=device_id,
            target=target,
...
def verify_bringup(
    ...
    """
    return _guard(
        lambda: workflows.verify_bringup(
            registry,
            sessions,
