"""Harness-owned, repo-scoped file tools for the agent bench.

NOT part of the MCP servers: boardex-target/boardex-logic expose no file
editing (the bench tool layer is "no code editing" by design), and the spec's
predecessor proof ran inside an editor that supplied file tools itself. The
harness owns that layer: list_files / read_file / write_file, scoped to the
run's board-profile ``repoPath``. write_file yields the contract's code_diff
artifact content ({files: [{path, reason, diff}]}) so edits render in the UI
diff viewer.
"""
...
class Workspace:
    def __init__(self, repo: Path) -> None:
        ...
        if not self.repo.is_dir():
            raise WorkspaceError(f"task repo {self.repo} is not a directory")
