"""MCP stdio client host.

Connects to boardex-target and boardex-logic exactly the way the repo's Cursor
...
class McpToolHost:
    ...
    async def connect(self, repo_root: Path) -> None:
        from mcp import ClientSession, StdioServerParameters
        from mcp.client.stdio import stdio_client

        self._stack = AsyncExitStack()
        for binary in SERVER_BINARIES:
            command = repo_root / ".venv" / "bin" / binary
