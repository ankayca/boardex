...
- **Left rail — Board Context:** compact card: board name, MCU, repo (basename), instrument list resolved by reference against the live bench (found = green dot + device id; degraded = the device's own StatusDot, amber offline / red error; missing = no dot, the profile's reference, amber "<reference> was not found on the bench"; serial resolves by kind, and with no bench snapshot every instrument reads offline — never an assumed online), safety line ("Flash requires approval · Max 3 iterations · Manual power: 3V3 confirmed"), "View details" → drawer with full profile incl. connection checklist.
