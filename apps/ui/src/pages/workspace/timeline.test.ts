...
// start order, pending plan rows for unexecuted plan indices, iteration dividers at
// the reducer's marker positions.
...
describe('deriveTimeline', () => {
  ...
  it('keeps both executed steps when two run steps share one plan index (§7.3 build & flash)', () => {
    ...
    expect(items[0]).toMatchObject({ step: { id: 'st_build', status: 'succeeded' } });
    expect(items[1]).toMatchObject({ step: { id: 'st_flash', status: 'active' } });
  });
  ...
  it('inserts the iteration divider exactly before the first iteration-2 step', () => {
    ...
    expect(items[1]).toEqual({ kind: 'iteration', iteration: 2, reason: 'applying address fix' });
  });
